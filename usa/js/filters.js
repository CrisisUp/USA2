...
function getRandomStateIdWithMedia() {
    const stateIds = Object.keys(stateData).filter((id) =>
        stateData[id] && stateData[id].media && stateData[id].media.length > 0
    );
...
export function initFilters() {
    ...
    document.getElementById('media-list').addEventListener('click', (event) => {
        ...
        favBtn.setAttribute('aria-label', isNowFav ? 'Remover dos favoritos' : 'Adicionar aos favoritos');
    });
}