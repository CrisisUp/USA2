...
import { initStatePopovers } from './display.js'; // Popovers e exibição de detalhes
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (!usaMap) {
      console.error(
        "Elemento SVG com ID 'usa-map' não encontrado após carregamento. Verifique o conteúdo do seu arquivo SVG e certifique-se que o ID é 'usa-map'."
      );
      mapContainer.innerHTML =
        '<p class="error-message">Erro: O mapa não pôde ser processado. Verifique o console para mais detalhes.</p>';
      return;
    }
    ...
    initMapInteractions(
      usaMap,
      stateData,
      stateId => setDisplayedState(stateId),
      statesList => resetStateHighlights(statesList)
    );
    ...
    // Inicializa Popover API para tooltips nos estados
    initStatePopovers(usaMap, stateData);
  } catch (error) {
    console.error('Erro na inicialização ou carregamento do mapa:', error);
    mapLoadingMessage.textContent =
